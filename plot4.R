...
## Make the plot and copy it into the png file
par(mfrow=c(2,2))
with(ElecDB,plot(datetime,Global_active_power, type = 'l',xlab = "",ylab = "Global Active Power"))
with(ElecDB,plot(datetime,Voltage, type = 'l'))
...
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c("black","red","blue"),lty=c(1,1,1),text.width = 90000,bty = 'n')
with(ElecDB,plot(datetime,Global_reactive_power, type = 'l'))
